...
THREE.TreeSpawner.prototype = {
    spawn : function(branch, extension, endSegments) {
        ...
        var branch = new THREE.TreeBranch({
            from : segment,
            rotation : rot,
            length : len,
        ...
        if (branch.generation === branch.generations) {
            endSegments.push(branch.to);
        }
        return branch;
    }
};
...
THREE.TreeBranch = function(params) {
    ...
    this.segments = this.buildTreeSegments(this.radius, this.radiusSegments, direction, this.heightSegments, params.endSegments);
    this.children = [];
}

THREE.TreeBranch.prototype = {

    buildTreeSegments : function(radius, radiusSegments, direction, heightSegments, endSegments) {
        ...
        segments.push(new THREE.TreeSegment(points[0], rotation, uvOffset, fromRadius, radiusSegments, endSegments));
        ...
        for(var i = 1; i < heightSegments; i++) {
            ...
            var segment = new THREE.TreeSegment(p0, rotation, uvOffset, radius, radiusSegments, endSegments);
    ...
    branch : function(spawner, count, endSegments) {
        for(var i = 0; i < count; i++) {
            // MEMO:
            //  at least one child is an extended branch.
            this.spawn(spawner, i == 0, endSegments);
        }
        this.children.forEach(function(child) {
            child.branch(spawner, count - 1, endSegments);
        });
    },
    ...
    spawn : function(spawner, extension, endSegments) {
        var child = spawner.spawn(this, extension, endSegments);
 ...
 * rotation : THREE.Matrix4
 * */
THREE.TreeSegment = function(position, rotation, uvOffset, radius, radiusSegments, endSegments) {
    ...
    this.build(radius, radiusSegments, endSegments);
}

THREE.TreeSegment.prototype = {

    build : function(radius, radiusSegments, endSegments) {
...
THREE.Tree = function(params, spawner) {
    ...
    this.root = new THREE.TreeBranch({ 
        ...
        endSegments: this.endSegments
    });

    this.spawner = spawner || new THREE.TreeSpawner();
    this.root.branch(this.spawner, this.generations, this.endSegments);
}
